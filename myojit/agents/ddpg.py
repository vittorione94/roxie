...
class DDPG(agent.Agent):
    ...
    def step(self, state: jnp.ndarray, evaluate: bool = False, key: jax.random.PRNGKey = None) -> jnp.ndarray:
        ...
        def add_noise(action, key):
            noise = jax.random.normal(key, action.shape) * self.exploration_noise
            return action + noise

        # Add exploration noise during training.
        # jax.lax.cond ensures this logic is JIT-compatible.
        action = jax.lax.cond(
            evaluate,
            lambda: action,  # If evaluate is True, return the action as is
            lambda: add_noise(action, key), # If evaluate is False, add noise
        )
    ...
    def update(self, prev_states, states, steps, key):
        ...
        #2. update critic
        def critic_loss_fn(critic_to_grad):
            # Get next actions from the TARGET actor, not the main one.
            next_actions = self.target_actor(samples['next_observations'], training=False)
            
            # Get Q-values for the next states from the TARGET critic.
            next_q_values = self.target_critic(samples['next_observations'], next_actions)
        ...
        self.soft_update(0.005, self.actor, self.target_actor)
        self.soft_update(0.005, self.critic, self.target_critic)
    ...
    def load(cls, path: Union[str, Path], actor: nnx.Module, critic: nnx.Module, replay: JaxReplayBuffer):
        ...
        actor = nnx.merge(actor, loaded_data['actor'])
        critic = nnx.merge(critic, loaded_data['critic'])
