...
@functools.partial(nnx.jit, static_argnames=(
    'gamma', 'gae_lambda', 'clip_eps', 'entropy_coef', 'value_coef', 'replay_get_fn'
))
def _grad_step(
    state: TrainState,
    ...
    _, advantages = jax.lax.scan(gae_scan_fn, jnp.zeros_like(bootstrap_value), deltas, reverse=True)
