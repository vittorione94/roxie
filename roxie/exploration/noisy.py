...
class NoiseModule(nnx.Module):
    ...
    @abc.abstractmethod
    def sample_noise(self, key: jax.random.PRNGKey, shape: Optional[tuple] = None) -> jnp.ndarray:
    ...
    def add_noise(self, 
                  ...
                  evaluation: bool = False) -> jnp.ndarray:
...
class ParameterNoise(nnx.Module):
    ...
    def perturb_parameters(self, 
                          model: nnx.Module, 
                          key: jax.random.PRNGKey) -> nnx.Module:
        """Create a perturbed copy of the model parameters."""
        import copy
        perturbed_model = copy.deepcopy(model)
        
        current_scale = self.decay_schedule(self.current_stddev.value, self.step_count.value)
...
class CompositeNoise(NoiseModule):
    ...
    def __init__(self, 
                 noise_modules: list[NoiseModule],
