...
OBS, ACT = 16, 6
# Divisible by every `num_minibatches` in the repo's PPO configs (4 and 20).
ENVS = 40
...
def _build(agent_cfg):
    ...
    cfg = OmegaConf.create(
        {"env": {"parallel_envs": ENVS}, "agent": agent_cfg}
    ).agent
...
def test_agent_config_constructs(path):
    assert _build(OmegaConf.load(path)) is not None
...
def test_default_optimizer_matches_the_previous_hardcoded_chain():
    ...
    grads = {"w": jnp.array([10.0, -4.0, 0.5])}
    ...
    # `None` (an agent built straight from Python, or an old checkpoint) keeps
    # the same behaviour.
    assert jnp.allclose(
        one_step(hardcoded),
...
def test_optimizer_block_selects_family_and_schedule():
    ...
    assert jnp.allclose(one_step(unclipped), jnp.array([-1.0, 0.4, -0.05]))


def test_network_rngs_offsets_are_distinct():
    import jax

    key = lambda offset: jax.random.key_data(network_rngs(0, offset).params.key.value)
    ...
    assert jnp.array_equal(key(0), jax.random.key_data(network_rngs(0, 0).params.key.value))
