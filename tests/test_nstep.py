...
truncations, done-on-first-step — plus n=1 flat-vs-trajectory parity and the
buffer layout the windows are read out of.

Driving a real agent over the n-step path is `test_agents.py`'s: D4PG and TD4
ship `n_step: 5`, so every learning pass there goes through `build_replay`'s trajectory
buffer and `repack_samples`.
"""
...
def _traj_sample(rewards, terminals, truncations):
    ...
    T = N + 1
    obs = jnp.tile(jnp.arange(
        1, T + 1, dtype=jnp.float32
    )[None, :, None], (1, 1, OBS_DIM))
    pad = lambda xs: jnp.array([list(xs) + [0.0] * (T - len(xs))], dtype=jnp.float32)
    return _FakeSample(experience=Transition(
        observation=obs,
        action=jnp.zeros((1, T, 2), dtype=jnp.float32),
        reward=pad(rewards),
...
class TestNStepRepack:
    ...
    def test_n1_matches_flat_pair_path(self):
        ...
        obs0 = jnp.full((1, OBS_DIM), 1.0, dtype=jnp.float32)
        obs1 = jnp.full((1, OBS_DIM), 2.0, dtype=jnp.float32)
        for term in (False, True):
            traj = _FakeSample(experience=Transition(
                observation=jnp.stack([obs0, obs1], axis=1),
                action=jnp.zeros((1, 2, 2), dtype=jnp.float32),
                reward=jnp.array([[3.0, 0.0]], dtype=jnp.float32),
                terminal=jnp.array([[term, False]]),
                truncation=jnp.array([[False, False]]),
            ))
            first = Transition(observation=obs0, action=jnp.zeros(
                (1, 2), dtype=jnp.float32
            ),
                               reward=jnp.array(
                                   [3.0], dtype=jnp.float32
                               ), terminal=jnp.array([term]))
            second = Transition(observation=obs1, action=jnp.zeros(
                (1, 2), dtype=jnp.float32
            ),
                                reward=jnp.array(
        ...
        for t in range(20):
            item = Transition(
                observation=jnp.array(
                    [[1000.0 + t], [2000.0 + t]], dtype=jnp.float32
                )[:, None, :],
                action=jnp.zeros((2, 1, 1), dtype=jnp.float32),
                reward=jnp.full((2, 1), float(t), dtype=jnp.float32),
