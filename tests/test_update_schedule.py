...
class _Schedule(Agent):
    """Bare carrier for the two hyperparameters `due_for_update` reads.

    Still an `Agent` subclass so it tracks the real `due_for_update`, but the
    contract's two abstract halves are stubbed to raise: the schedule is the
    subject here, and nothing in this file acts or learns.
    """

    def __init__(self, before, between):
        self.hp = OffPolicyHyperparams(
            memory_warmup=before, steps_between_updates=between
        )

    def select_action(self, *_args, **_kwargs):
        raise NotImplementedError

    def _compile_and_run(self, *_args, **_kwargs):
        raise NotImplementedError
...
def _fire_count(before, between, stride, total, start=None):
    """Learning passes a trainer loop fires stepping `stride` at a time up to `total`."""
...
class TestUnalignedOffset:
    ...
    def test_fires_for_every_offset_stride_pair(self):
        """No (offset, stride) combination may silently disarm the schedule.

        One test over the grid rather than a parametrized cell each: they share
        a single arithmetic claim, and a failure names the pair it failed on.
        """
        for before in (0, 1, 47, 30_000, 30_720, 99_999):
            for stride in (1, 100, 256, 400, 1_000):
                total = before + 200 * 2_048
                assert _fire_count(before, 2_048, stride, total) > 0, (
                    f"memory_warmup={before} with parallel_envs={stride} "
                    f"fired no updates"
                )


class TestRate:
    def test_one_pass_per_window(self):
    ...
    def test_bench_offsets_are_multiples_of_parallel_envs(self):
        ...
        for path in glob.glob("experiments/dmc/agent/*_bench.yaml"):
            ...
            for key in ("memory_warmup", "steps_between_updates"):
                # Indented: they live inside the `hyperparams:` block.
                m = re.search(rf"^\s+{key}: ([\d_]+)", text, re.M)
                if m is None:
