"""Which policy gets published, and the config couplings the 1B budget created.
...
The mocap benchmark's exploration anneal is measured in env steps, so it is tied
to `trainer.steps` — at 40% of the budget by its own design note. Raising the
budget without moving it turns the anneal into "no exploration for 96% of the
...
MANIFEST_ROWS = [
    # status, suite, cell, agent, steps
    ("ok", "mocap_cmu_006_13", "warp_gpu", "td3", "1000000000"),
    ("fail", "mocap_cmu_006_13", "warp_gpu", "sac", "1000000000"),
    ("ok", "mocap_cmu_006_13", "warp_gpu", "ppo", "50000000"),
    ("ok", "mocap_cmu_006_13", "envpool_gpu", "td3", "1000000000"),
    ("ok", "walker_walk", "warp_gpu", "td3", "5000000"),
]
...
def _manifest(tmp_path):
    ...
    with path.open("w", newline="") as handle:
        ...
        writer.writerow(["status", "suite", "cell", "agent", "steps",
                         "seconds", "sps", "run_dir"])
        for status, suite, cell, agent, steps in MANIFEST_ROWS:
            writer.writerow([status, suite, cell, agent, steps, "1", "1",
                             f"outputs/{suite}/{cell}/{agent}/x"])
...
def test_steps_filter_excludes_a_shorter_pilot(tmp_path):
    """PPO's `ok` row is a 50M pilot; asking for 1B must not publish it as one."""
    runs = export.latest_ok_runs(_manifest(tmp_path), "mocap_cmu_006_13",
                                 "warp_gpu", steps=1_000_000_000)
    assert set(runs) == {"td3"}
...
def test_other_cells_and_suites_are_excluded(tmp_path):
    ...
    assert all(row["suite"] == "mocap_cmu_006_13" for row in runs.values())
...
def test_the_newest_run_of_an_agent_wins(tmp_path):
    ...
    with path.open("w", newline="") as handle:
        ...
        for stamp in ("first", "second"):
            writer.writerow(["ok", "mocap_cmu_006_13", "warp_gpu", "td3",
                             "1000000000", "1", "1", f"outputs/{stamp}"])
    runs = export.latest_ok_runs(export.read_manifest(path), "mocap_cmu_006_13",
                                 "warp_gpu", steps=None)
...
BENCH = OmegaConf.load(REPO / "experiments" / "mocap" / "bench" / "cmu_006_13.yaml")
NOISE = OmegaConf.load(REPO / "experiments" / "mocap" / "noise" / "bench_gaussian.yaml")


def test_mocap_noise_anneal_tracks_the_step_budget():
    """`decay_steps` is 40% of `trainer.steps` by design (see the header of
    experiments/mocap/noise/bench_gaussian.yaml). A decay measured in env steps
...
def test_mocap_save_cadence_divides_the_budget():
...
def test_mocap_run_produces_enough_checkpoints_to_choose_from():
